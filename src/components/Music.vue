<template>
  <div class="music">
    ...
    <div class="center">
      ...
      <el-table :data="musicList.songs" stripe style="width: 100%; min-height: 500px;">
        <el-table-column label="序号" width="180">
          <template slot-scope="scope">
            <span>{{scope.$index+1+(currentPage-1)*30}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="歌手" width="180" prop="artists[0].name">
          <template slot-scope="scope">
        ...
        <el-table-column label="专辑" width="200">
          <template slot-scope="scope">
            <span @click="getAlbum(scope.row.album.id)" class="music_name">{{scope.row.album.name}}</span>
          </template>
        </el-table-column>
        <el-table-column label="时长" width="200">
          <template slot-scope='scope'>
            <span>{{Math.floor((scope.row.duration/1000/60))}}:{{Math.floor((scope.row.duration/1000) % 60)}}</span>
          </template>
        </el-table-column>
      </el-table>
      <br>
      <el-pagination class="center" :current-page="currentPage" background style="width: 30%;" layout="prev, pager, next" @current-change="handleCurrentChange"
